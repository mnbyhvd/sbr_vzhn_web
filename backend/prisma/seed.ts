...
async function main() {
  ...
  await prisma.vacancyResponse.deleteMany({});
  await prisma.projectDirection.deleteMany({});
  ...
  await prisma.internationalExperience.deleteMany({});
  ...
  await prisma.project.createMany({ data: [
    {
      ...
      details: 'Полный цикл разработки CRM-системы с интеграцией платежных шлюзов.',
      bgColor: '#ffffff',
      textColor: '#222222',
    ...
    },
    {
      ...
      details: 'Аудит инфраструктуры, миграция в облако, повышение производительности на 40%.',
      bgColor: '#ffffff',
      textColor: '#222222',
  ...
  // Vacancy Categories
  const categories = await prisma.vacancyCategory.createMany({
    data: [
      { name: 'Frontend' },
      { name: 'Backend' },
      { name: 'DevOps' },
  ...
  const frontendCategory = await prisma.vacancyCategory.findUnique({ where: { name: 'Frontend' } });
  const backendCategory = await prisma.vacancyCategory.findUnique({ where: { name: 'Backend' } });
  ...
  const frontendVacancy = await prisma.vacancy.findFirst({ where: { title: { contains: 'Frontend' } } });
  const backendVacancy = await prisma.vacancy.findFirst({ where: { title: { contains: 'Backend' } } });

  // Vacancy Responses (отклики)
  if (frontendVacancy && backendVacancy) {
    await prisma.vacancyResponse.createMany({
      data: [
        {
  ...
  await prisma.direction.createMany({
    data: [
      {
        title: 'Финтех',
        description: 'Ведение портфолио, учет операций, контроль рисков для инвесторов',
        gridSize: 1,
        textColor: '#1A59DE',
        bgColor: '#fff',
      },
      ...
      },
      {
        ...
        description: 'Разработка ПО для считывания радужной оболочки глаза и систем доступа',
        gridSize: 2,
        textColor: '#fff',
        bgColor: '#1A59DE',
      },
      {
        ...
        description: 'Разработка программного комплекса мониторинга сердечно-сосудистой системы',
        gridSize: 1,
        textColor: '#1A59DE',
        bgColor: '#fff',
      },
  ...
  if (allProjects.length >= 2 && allDirections.length >= 4) {
    ...
    await prisma.projectDirection.createMany({
      data: [
