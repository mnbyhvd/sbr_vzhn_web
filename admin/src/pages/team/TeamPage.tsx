    ...
    >
      <Avatar
        src={member.image && member.image.length > 0
          ? (member.image.startsWith('http') ? member.image : `http://localhost:3001${member.image}`)
          : undefined}
        alt={member.name}
        sx={{ 
          width: 50, 
...
const TeamPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      if (editingMember) {
        await fetch(`http://localhost:3001/api/team/${editingMember.id}`, {
          method: 'PUT',
  ...
  const confirmDelete = async () => {
    if (!deleting) return;
    try {
      await fetch(`http://localhost:3001/api/team/${deleting.id}`, {
        method: 'DELETE',
      });
      setDeleting(null);
      fetchTeamMembers();
    ...
    if (active.id !== over?.id) {
      setTeamMembers((items) => {
        ...
        newItems.forEach(async (item, index) => {
          try {
            await fetch(`http://localhost:3001/api/team/${item.id}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
