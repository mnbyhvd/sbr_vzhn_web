    ...
    >
      <Avatar
        src={partner.logo && partner.logo.length > 0
          ? (partner.logo.startsWith('http') ? partner.logo : `http://localhost:3001${partner.logo}`)
          : undefined}
        alt={partner.name}
        sx={{ 
          width: 50, 
...
const PartnersPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      if (editingPartner) {
        await fetch(`http://localhost:3001/api/partners/${editingPartner.id}`, {
          method: 'PUT',
  ...
  const confirmDelete = async () => {
    if (!deleting) return;
    try {
      await fetch(`http://localhost:3001/api/partners/${deleting.id}`, {
        method: 'DELETE',
      });
      setDeleting(null);
      fetchPartners();
    ...
    if (active.id !== over?.id) {
      setPartners((items) => {
        ...
        newItems.forEach(async (item, index) => {
          try {
            await fetch(`http://localhost:3001/api/partners/${item.id}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
