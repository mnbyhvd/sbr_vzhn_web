    ...
    >
      <Avatar
        src={vacancy.image && vacancy.image.length > 0
          ? (vacancy.image.startsWith('http') ? vacancy.image : `http://localhost:3001${vacancy.image}`)
          : undefined}
        alt={vacancy.title}
        sx={{ 
          width: 50, 
...
const VacanciesPage: React.FC = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/vacancies/categories');
      const data = await response.json();
      if (Array.isArray(data)) {
        setCategories(data);
  ...
  const handleSubmit = async () => {
    try {
      if (editingVacancy) {
        await fetch(`http://localhost:3001/api/vacancies/${editingVacancy.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
      } else {
        await fetch('http://localhost:3001/api/vacancies', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
  ...
  const confirmDelete = async () => {
    if (!deleting) return;
    try {
      await fetch(`http://localhost:3001/api/vacancies/${deleting.id}`, {
        method: 'DELETE',
      });
      setDeleting(null);
      fetchVacancies();
    ...
    if (active.id !== over?.id) {
      setVacancies((items) => {
        ...
        newItems.forEach(async (item, index) => {
          try {
            await fetch(`http://localhost:3001/api/vacancies/${item.id}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
  ...
  const handleOpenResponses = async (vacancy: Vacancy) => {
    ...
    const res = await fetch(`http://localhost:3001/api/vacancies/${vacancy.id}/responses`);
